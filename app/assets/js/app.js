(() => {
    ...
    function formSnapshot(form, excludeCsrf = false) {
        return [...new FormData(form).entries()]
            .filter(([name]) => !excludeCsrf || name !== "csrfmiddlewaretoken")
            .map(([name, value]) => {
                const normalized = value instanceof File
                    ? `${value.name}:${value.size}:${value.lastModified}`
                    : String(value);

                return `${encodeURIComponent(name)}=${encodeURIComponent(normalized)}`;
            })
            .join("&");
    }

    function initPhotoEditors() {
        qsa("[data-photo-editor]").forEach((editor) => {
            const input = qs("[data-photo-input]", editor);
            const remove = qs("[data-photo-remove]", editor);
            const preview = qs("[data-photo-preview]", editor);
            const fallback = qs("[data-photo-fallback]", editor);
            const filename = qs("[data-photo-name]", editor);
            ...
            const showFallback = (message) => {
                ...
                if (filename && message) filename.textContent = message;
            };

            const showImage = (source, message) => {
                preview.src = source;
            ...
            input.addEventListener("change", () => {
                ...
                if (!file) {
                    if (originalSource && !remove?.checked) {
                        showImage(originalSource, "Foto actual guardada");
                    } else {
                        showFallback("Aún no se ha seleccionado una foto");
                    }
            ...
            remove?.addEventListener("change", () => {
                if (remove.checked) {
                    input.value = "";
                    showFallback("La foto se quitará al guardar");
    ...
    function initProfileForm() {
        ...
        const snapshot = () => formSnapshot(form, true);
    ...
    function initUserAdminForm() {
        ...
        form.addEventListener("input", (event) => {
            ...
            const currentState = formSnapshot(form);
